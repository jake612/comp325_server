...
def weighting(c):  # c raw/un-normalized scores for every therapist
    ...
    for i in range(len(c)):
        ...
        if (val[0] == -1):
            ...
            distance = (1 - (val[0] / c[maximum][0])) * w
        ...
        budget = (val[2] / customer_range) * w

        # Qualifications
        # value is either 1 or 0, yes or no
        qualifications = val[3] * w

        # score for therapist
        score[i] = [distance + insurance + budget + qualifications]
...
def get_rank_info_and_map(dict_fields):
    ...
    def best_match(address, insurance, minCost, maxCost, qualifications):
        locator = Nominatim(user_agent='myGeocoder')
    ...
    ranking = best_match(customer_location, insurance, minimum_cost, maximum_cost, qualifications)
    print(ranking)
